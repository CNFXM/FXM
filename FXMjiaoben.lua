...
about:Slider('触碰范围', 'Sliderflag', 100, 1, 100,false, function(Value)
        game.Players.LocalPlayer.Character.Humanoid.MaxHealth = Value_G.HeadSize = Value
_G.Disabled = true
...
if v.Name ~= game:GetService('Players').LocalPlayer.Name then
pcall(function()
    -- 新增：输入0时自动将尺寸替换为200，正数保持原输入值
    local finalSize = _G.HeadSize <= 1 and 0.01 or _G.HeadSize
    
    -- 原有触碰箱及属性代码完全保留，仅将尺寸参数替换为finalSize
    v.Character.HumanoidRootPart.Size = Vector3.new(finalSize, finalSize, finalSize)
    -- 新增：输入0时视觉隐藏，正数时恢复原透明度（0.7）
    v.Character.HumanoidRootPart.Transparency = _G.HeadSize <= 0 and 1 or 0.7
    v.Character.HumanoidRootPart.BrickColor = BrickColor.new("Really red")
    v.Character.HumanoidRootPart.Material = "Neon"
    -- 原有触碰箱核心代码（CanCollide）未做任何修改，完全保留源码
    v.Character.HumanoidRootPart.CanCollide = false
end)
end
end
