...
-- 2. 坐标显示与复制（仅复制纯坐标数值，格式：X,Y,Z）
local coordLabel = about:Label("坐标为:加载中...", true)
local currentCoordStr = "加载中..." -- 存储纯坐标字符串，用于复制
local RunService = game:GetService("RunService") -- 引入RunService服务
local LocalPlayer = game:GetService("Players").LocalPlayer -- 引入本地玩家

-- 实时更新坐标（同步标签文本和纯坐标字符串，修复空值风险）
...
updateCoordLabel() -- 初始执行一次，避免等待首帧更新

-- 窗口关闭时清理连接，防止内存泄漏
win:OnClose(function()
    coordUpdateConn:Disconnect()
end)
