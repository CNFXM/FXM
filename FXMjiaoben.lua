...
about:Slider('触碰范围', 'Sliderflag', 0, 0, 100,false, function(Value)
        _G.HeadSize = Value
    _G.Disabled = true 
...
local UITab99 = win:Tab("传送功能",'7734068321')

local about = UITab99:section("传送",true)
...
-- 下拉菜单选择目标玩家
about:Dropdown("选择传送目标玩家", "Player Name", AllPlayers, function(Value)
...
local function TeleportToPlayer(player)
    if player and player.Character and player.Character.PrimaryPart then
        game.Players.LocalPlayer.Character:PivotTo(player.Character.PrimaryPart.CFrame)
    end
end
...
local function startLoopTeleport()
    ...
    -- 每1秒执行一次传送（可调整间隔）
    loopTimer = game:GetService("RunService").Heartbeat:Connect(function()
