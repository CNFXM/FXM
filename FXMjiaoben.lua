...
about:Toggle("把玩家传送过来", function(Value)
    -- 用局部变量存储当前线程，避免重复创建
    local currentThread = nil
    
    if Value then
        -- 开启时启动循环
        currentThread = spawn(function()
            while Value do  -- 直接使用开关的实时状态（Value）
                local localPlayer = game.Players.LocalPlayer
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied)
                ...
                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- 失败通知添加冷却，避免刷屏
                    Notify("Y", "无法传送 玩家已消失或角色未加载", "rbxassetid://", 5)
                    wait(3)  -- 失败后延长等待时间
                    continue  -- 跳过本次循环剩余等待
                end
                wait(1)  -- 正常间隔
            end
            Notify("L", "已停止传送玩家", "rbxassetid://", 5)
        end)
    else
        -- 关闭时无需额外操作，循环会因Value为false自动停止
        Notify("L", "已停止传送玩家", "rbxassetid://", 5)
    end
end)
