...
-- 步骤一：调节范围大小（仅更新数值，不立即生效）
about:Slider('触碰范围', 'Sliderflag', 1, 0, 100,false, function(Value)
    -- 仅更新全局变量存储的范围值，不执行实际修改操作
    _G.HeadSize = Value
end)

-- 步骤二：点击按钮刷新，开启新的范围大小（触发实际修改逻辑）
about:Button("刷新开启范围",function()
    -- 点击后设置禁用状态，避免重复执行
    _G.Disabled = true
    
    -- 绑定渲染步进事件，执行范围生效逻辑（仅修改隐藏规则）
    game:GetService('RunService').RenderStepped:connect(function()
        if _G.Disabled then
            for i,v in next, game:GetService('Players'):GetPlayers() do
                if v.Name ~= game:GetService('Players').LocalPlayer.Name then
                    pcall(function()
                        -- 核心修改：仅输入1时隐藏，输入2-100时显示（不修改范围值本身）
                        local finalSize = _G.HeadSize -- 保留滑块输入的原始范围值
                        local isHidden = _G.HeadSize == 0 -- 仅值为1时触发隐藏
                        
                        -- 触碰箱属性：按新隐藏规则设置透明度，其余代码完全保留
                        v.Character.HumanoidRootPart.Size = Vector3.new(finalSize, finalSize, finalSize)
                        v.Character.HumanoidRootPart.Transparency = isHidden and 1 or 0.7 -- 仅此处修改
