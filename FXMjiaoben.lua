...
about:Toggle("自动锻炼","Toggle",false,function(Value)
    Jump = Value -- 开关状态赋值
    -- 用 spawn 开启独立线程，避免阻塞主线程
    spawn(function()
        while Jump do -- 关键：用 Jump 作为循环条件，关闭时自动终止
            local args = {[1] = "rep"}
            -- 加保护判断，避免事件未加载时报错
...
-- 提前声明局部变量：存储开关状态 + 挥拳线程句柄（避免全局污染）
local AutoPunchEnabled = false
local PunchThread = nil

about:Toggle("自动挥拳","Toggle",false,function(Value)
    AutoPunchEnabled = Value -- 更新开关状态
    
    -- 关键：先终止旧线程（避免多线程残留）
    if PunchThread then
        task.cancel(PunchThread) -- 终止之前的挥拳线程（用task库更高效，替代wait）
        PunchThread = nil -- 清空句柄
    end
    
    -- 开关开启时，启动新的挥拳线程
    if AutoPunchEnabled then
        PunchThread = task.spawn(function() -- 用task.spawn替代spawn，返回线程句柄
            while AutoPunchEnabled do -- 循环条件绑定开关状态
                local args = {
                    [1] = "punch",
                    [2] = "rightHand"
                }
                ...
                task.wait(0.1) -- 用task.wait替代wait，支持线程取消（关键！）
            end
        end)
    end
end)
...
about:Button("自动宝箱",function()
    ...
    spawn(function()
        ...
        for i, targetCFrame in ipairs(teleportPoints) do
            rootPart.CFrame = targetCFrame  -- 传送到宝箱位置
            task.wait(0.5)  -- 停留0.5秒（等待宝箱加载/交互生效）

            -- （可选）自动触发宝箱交互（需根据游戏内宝箱逻辑调整）
            -- 示例1：若宝箱是Part，通过触碰/点击触发，可模拟点击（需替换为游戏实际事件）
            -- local chest = workspace:FindFirstChild("宝箱名称", true) -- 查找宝箱
            -- if chest then
            --     game:GetService("ReplicatedStorage").OpenChest:FireServer(chest) -- 触发打开事件
            -- end
        end
    end)
end)

-- 关键：显示UI窗口（之前的代码漏掉了，不加窗口不会显示！）
win:Show()
