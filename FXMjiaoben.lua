...
about:Toggle("循环拉人", "loopPull", false, function(IsOn) -- 用IsOn表示开关状态，更直观
    -- 停止旧循环，防止重复启动（保留原关键逻辑）
    if pullLoopTask then task.cancel(pullLoopTask) end

    -- 开关开启：启动你要的while IsOn循环
    if IsOn then
        pullLoopTask = task.spawn(function()
            -- 替换为你指定的while IsOn循环结构
            while IsOn do
                -- 核心拉人源码（完全保留原拉人逻辑）
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied)
