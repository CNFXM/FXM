...
-- Toggle开关：浮窗坐标仪（开关控制启停+关闭浮窗自动同步）
about:Toggle("浮窗坐标仪","coordDisplay",true,function(Value)
    local Players = game:GetService("Players")
    ...
    local function createCoordUI()
        ...
        copyBtn.MouseButton1Click:Connect(function()
            if setclipboard and coordSystem.currentPos then
                local coordStr = string.format("X: %.2f, Y: %.2f, Z: %.2f",
                    coordSystem.currentPos.X, coordSystem.currentPos.Y, coordSystem.currentPos.Z
                )
                setclipboard(coordStr)
                -- 简化通知（无Rayfield时用基础提示，有则用原通知）
                if typeof(Rayfield) == "table" and Rayfield.Notify then
                    Rayfield:Notify({Title = "复制成功", Content = coordStr, Duration = 2})
                end
            end
        end)
        ...
        -- 【核心】关闭浮窗按钮（点击关闭后同步Toggle开关）
        local closeBtn = Instance.new("TextButton")
        closeBtn.Size = UDim2.new(0, 15, 0, 15)
        closeBtn.Position = UDim2.new(0, 185, 0, 0) -- 显示框右上角
        ...
        -- 点击关闭浮窗：同步关闭Toggle开关（设置Value为false）
        closeBtn.MouseButton1Click:Connect(function()
            coordSystem.isEnabled = false
            container.Visible = false
            if coordSystem.updateConn then
                coordSystem.updateConn:Disconnect()
            end
            -- 同步Toggle开关状态（关键：让开关显示为关闭）
            for _, tab in ipairs(about.Tabs) do
                for _, element in ipairs(tab.Elements) do
                    if element.Name == "浮窗坐标仪" and element.Type == "Toggle" then
     ...
     local function setEnabled(state)
         ...
         -- 首次启动时创建UI，后续仅控制显示/隐藏
         if not coordSystem.gui then
             coordSystem.gui = createCoordUI()
         end
         coordSystem.gui.container.Visible = state
         -- 启停坐标更新
         if state then
             if not coordSystem.updateConn or coordSystem.updateConn.Connected == false then
                 coordSystem.updateConn = RunService.Heartbeat:Connect(updateCoord)
                 updateCoord()
             end
         else
             if coordSystem.updateConn and coordSystem.updateConn.Connected then
                 coordSystem.updateConn:Disconnect()
             end
         end
     end
     ...
     player.CharacterAdded:Connect(function()
         if coordSystem.isEnabled then
             updateCoord() -- 角色重生后重新获取坐标
         end
     end)
     -- 5. 开关状态变化时执行
     setEnabled(Value)
