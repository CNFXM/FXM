...
-- Toggle开关：完整保留原Ragdoll功能（关闭时人物起身，无源码修改）
about:Toggle("人物摔倒","ragdoll",false,function(Value)
    -- 原源码完整迁移：保留所有变量、函数、逻辑
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService") -- 原源码保留，虽未使用但不删除
    local player = Players.LocalPlayer

    -- 原GUI相关（仅创建不显示，用开关控制功能，保留原变量避免逻辑报错）
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "RagdollGui"
    screenGui.ResetOnSpawn = false
    screenGui.Parent = player:WaitForChild("PlayerGui")
    screenGui.Visible = false -- 隐藏原GUI，避免与开关冲突
    ...
    button.Visible = false -- 隐藏原按钮，用开关控制

    -- 原状态变量（保留）
    local isRagdolled = Value -- 绑定Toggle开关状态
    local motorBackup = {}
    ...
    local function toggleRagdoll()
        ...
        if not isRagdolled then
            ...
            isRagdolled = true
            button.Text = "Unragdoll" -- 保留原按钮文字修改，不影响开关功能
            ...
            isRagdolled = false
            button.Text = "Toggle Ragdoll" -- 保留原按钮文字修改
        end
    end
    ...
    -- 开关状态变化时执行布娃娃切换（核心：用开关触发原toggleRagdoll函数）
    toggleRagdoll()
end)
