...
UITab1:Toggle("移除UI辉光", "", false, function(state)
        if state then
...
UITab1:Toggle("彩虹UI", "", false, function(state)
        if state then
...
UITab1:Button("摧毁GUI",function()
            game:GetService("CoreGui")["frosty is cute"]:Destroy()
        end)

UITab1:Keybind("切换用户界面", Enum.KeyCode.Home, function(Value)
            ToggleUILib()
        end)
