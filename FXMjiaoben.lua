...
-- 假设已通过 library.new 创建窗口并获取到 section（即原代码中的 about）
-- 1. 速度调节滑块（仅负责调节速度值，不直接控制生效）
about:Slider("调节速度值", "WalkSpeed_Value", game.Players.LocalPlayer.Character.Humanoid.WalkSpeed, 0, 1000, false, function(Speed)
    -- 仅将调节后的速度值存入 flags，不实时赋值
    library.flags["WalkSpeed_Value"] = Speed
end)

-- 2. 速度开关按钮（控制速度是否生效）
about:Toggle("速度开关", "WalkSpeed_Toggle", false, function(IsEnabled)
    spawn(function()
        while task.wait() do
            local humanoid = game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if not humanoid then return end -- 防止角色加载问题
            
            -- 开关启用时，赋值为滑块调节的速度；关闭时重置为默认（这里默认设为16，可自行修改）
            humanoid.WalkSpeed = IsEnabled and library.flags["WalkSpeed_Value"] or 16
        end
    end)
end)
