...
about:Button("传送到玩家旁边", function()
    ...
    if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        Notify("Y脚本", "无法传送 玩家已消失或角色未加载", "rbxassetid://", 5)
    end
end)

-- 传送到玩家旁边（开关版）
local tpToPlayerActive = false
about:Toggle("传送到玩家旁边", "tpToPlayerFlag", false, function(state)
    tpToPlayerActive = state
    if state then
        -- 开启时循环执行传送
        spawn(function()
            while tpToPlayerActive do
                local localPlayer = game.Players.LocalPlayer
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied)
                ...
                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    HumRoot.CFrame = tp_player.Character.HumanoidRootPart.CFrame + Vector3.new(0, 3, 0)
                    Notify("Y脚本", "已传送到玩家身边（持续模式）", "rbxassetid://", 2)
                else
                    Notify("Y脚本", "无法传送 玩家已消失或角色未加载", "rbxassetid://", 2)
                    -- 若目标无效，自动关闭开关
                    tpToPlayerActive = false
                    -- 这里需要根据框架是否支持手动修改Toggle状态来决定是否添加，若支持可补充：
                    -- about:SetToggle("tpToPlayerFlag", false)
                end
                wait(0.5) -- 间隔时间（可调整）
            end
            Notify("Y脚本", "已关闭传送到玩家旁边", "rbxassetid://", 5)
        end)
    end
end)

-- 把玩家传送过来（开关版）
local tpHereActive = false
about:Toggle("把玩家传送过来", "tpHereFlag", false, function(state)
    tpHereActive = state
    if state then
        -- 开启时循环执行传送
        spawn(function()
            while tpHereActive do
                local localPlayer = game.Players.LocalPlayer
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied)
                ...
                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    Notify("L", "已将玩家传送过来（持续模式）", "rbxassetid://", 2)
                else
                    Notify("Y", "无法传送 玩家已消失或角色未加载", "rbxassetid://", 2)
                    -- 若目标无效，自动关闭开关
                    tpHereActive = false
                    -- 同理，若支持可补充：
                    -- about:SetToggle("tpHereFlag", false)
                end
                wait(0.5) -- 间隔时间（可调整）
            end
            Notify("L", "已关闭把玩家传送过来", "rbxassetid://", 5)
        end)
    end
end)
...
about:Toggle("查看玩家", 'Toggleflag', false, function(state)
    ...
    if state then
        if tp_player and tp_player.Character and tp_player.Character:FindFirstChild("Humanoid") then
            ...
            Notify("Y", "无法切换视角 玩家不存在或角色未加载", "rbxassetid://", 5)
        end
    else
