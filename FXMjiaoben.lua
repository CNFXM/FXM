...
local localPlayer = game.Players.LocalPlayer
local loopTask -- 控制循环的任务变量

-- 开关控制循环传送
about:Toggle("循环传送", "loopTP", false, function(Value)
    -- 停止旧循环，防止重复
    ...
    if Value then
        loopTask = task.spawn(function()
            while true do -- 持续循环
                -- 你的传送核心代码（原逻辑保留）
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied) -- 确保playernamedied是已定义的玩家名变量

                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    HumRoot.CFrame = tp_player.Character.HumanoidRootPart.CFrame + Vector3.new(0, 3, 0)
                    Notify("FXM脚本", "已循环传送至玩家身边", "rbxassetid://", 5)
                else
                    Notify("FXM脚本", "循环传送失败：玩家消失或角色未加载", "rbxassetid://", 5)
                end

                task.wait(1) -- 循环间隔1秒（可调整）
            end
        end)
    end
end)
...
-- ###########################################################################
-- 4. 循环拉人功能（把选中玩家传送到自己身边，带开关控制）
-- ###########################################################################
local localPlayer = game.Players.LocalPlayer
local pullLoopTask -- 控制循环拉人的任务变量

-- 循环拉人开关（和循环传送共用下拉菜单选的playernamedied）
about:Toggle("循环拉人", "loopPull", false, function(Value)
    -- 停止旧循环，防止重复启动
    if pullLoopTask then task.cancel(pullLoopTask) end

    -- 开关开启：启动循环拉人
    if Value then
        pullLoopTask = task.spawn(function()
            while true do
                -- 核心拉人逻辑（复用你的代码，适配循环）
                -- 先获取自己的HumanoidRootPart（加判断，避免角色未加载报错）
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied) -- 读取下拉菜单选中的玩家

                -- 拉人判断（优化了“自己的角色是否加载”的判断）
                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    -- 把目标玩家传送到自己上方3格
                    tp_player.Character.HumanoidRootPart.CFrame = HumRoot.CFrame + Vector3.new(0, 3, 0)
                    Notify("FXM", "已循环拉人：" .. playernamedied, "rbxassetid://", 3)
                else
                    -- 更友好的失败提示（区分“没选玩家”和“玩家消失”）
                    local tip = ""
                    if not playernamedied then
                        tip = "请先从下拉菜单选择玩家"
                    elseif not tp_player or not tp_player.Character then
                        tip = "玩家已消失或角色未加载"
                    else
                        tip = "目标玩家缺少HumanoidRootPart"
                    end
                    Notify("FXM", "循环拉人失败：" .. tip, "rbxassetid://", 3)
                end

                task.wait(0.1) -- 循环间隔1秒（可调整，如2=每2秒拉一次）
            end
        end)
    end
end)
