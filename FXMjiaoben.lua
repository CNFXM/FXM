...
about:Button("传送到玩家旁边", function()
    ...
    if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        Notify("FXM", "已经传送到玩家身边", "rbxassetid://", 5)
    else
        Notify("FXM", "无法传送 玩家已消失或角色未加载", "rbxassetid://", 5)
    end
end)
...
about:Toggle("循环传送到玩家旁边", function(Value)
    isTpingToPlayer = Value -- 直接关联开关状态（Value为true时开启，false时关闭）
    
    if isTpingToPlayer then
        -- 开关开启时启动循环
        Notify("FXM", "开始循环传送到玩家身边", "rbxassetid://", 5)
        
        tpLoop = spawn(function()
            while isTpingToPlayer do
                local localPlayer = game.Players.LocalPlayer
                local HumRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                local tp_player = game.Players:FindFirstChild(playernamedied)
                ...
                if HumRoot and tp_player and tp_player.Character and tp_player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- 失败时提示一次，避免刷屏
                    if not (tp_player and tp_player.Character) then
        ...
        -- 开关关闭时停止循环
        Notify("FXM", "已停止循环传送", "rbxassetid://", 5)
        if tpLoop then
            coroutine.close(tpLoop) -- 终止线程
        end
    end
end)
