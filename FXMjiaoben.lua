...
-- 1. 坐标显示与复制（修复空值风险+优化复制体验）
local coordLabel = about:Label("坐标为:加载中...", true)
local currentCoordStr = "加载中..."
local function updateCoordLabel()
    local Character = LocalPlayer.Character
    if not Character then
        coordLabel.Text = "坐标为:等待角色加载..."
        currentCoordStr = "等待角色加载..."
        return
    end
    local HumanoidRootPart = Character:FindFirstChild("HumanoidRootPart")
    if HumanoidRootPart then
        local x = string.format("%.2f", HumanoidRootPart.Position.X)
        local y = string.format("%.2f", HumanoidRootPart.Position.Y)
        local z = string.format("%.2f", HumanoidRootPart.Position.Z)
        coordLabel.Text = "坐标为:" .. x .. ", " .. y .. ", " .. z
        currentCoordStr = x .. "," .. y .. "," .. z
    else
        coordLabel.Text = "坐标为:等待角色加载..."
        currentCoordStr = "等待角色加载..."
    end
end
about:Button("复制坐标",function()
    if setclipboard and currentCoordStr ~= "等待角色加载..." and currentCoordStr ~= "加载中..." then
        setclipboard(currentCoordStr)
        ui:Notify({Title = "复制成功", Content = "坐标已复制：" .. currentCoordStr, Duration = 2})
    else
        ui:Notify({Title = "无法复制", Content = "请等待角色加载完成后再尝试", Duration = 2})
    end
end)
local coordUpdateConn = RunService.Heartbeat:Connect(updateCoordLabel)
updateCoordLabel()

-- 2. 账号年龄（显示+复制）
local accountAgeText = "你的账号年龄:"..LocalPlayer.AccountAge.."天"
about:Label(accountAgeText)
about:Button("复制账号年龄",function()
    setclipboard(accountAgeText)
    ui:Notify({Title = "复制成功", Content = "账号年龄已复制", Duration = 2})
end)

-- 3. 注入器（显示+复制，兼容identifyexecutor()接口）
local executorText = "你的注入器:"..(identifyexecutor() or "未识别")
about:Label(executorText)
about:Button("复制注入器",function()
    setclipboard(executorText)
    ui:Notify({Title = "复制成功", Content = "注入器信息已复制", Duration = 2})
end)

-- 4. 用户名（显示+复制）
local userNameText = "你的用户名:"..LocalPlayer.Name
about:Label(userNameText)
about:Button("复制用户名",function()
    setclipboard(userNameText)
    ui:Notify({Title = "复制成功", Content = "用户名已复制", Duration = 2})
end)

-- 5. 服务器名称（显示+复制，修复获取逻辑）
local serverNameText = "你现在的服务器名称:"..MarketplaceService:GetProductInfo(game.PlaceId).Name
about:Label(serverNameText)
about:Button("复制服务器名称",function()
    setclipboard(serverNameText)
    ui:Notify({Title = "复制成功", Content = "服务器名称已复制", Duration = 2})
end)
...
about:Button("复制服务器ID",function()
    ...
    ui:Notify({Title = "复制成功", Content = "服务器ID已复制", Duration = 2})
end)

-- 7. 用户ID（显示+复制）
local userIdText = "你的用户ID:"..LocalPlayer.UserId
about:Label(userIdText)
about:Button("复制用户ID",function()
    setclipboard(userIdText)
    ui:Notify({Title = "复制成功", Content = "用户ID已复制", Duration = 2})
end)

-- 8. 客户端ID（显示+复制）
local clientIdText = "获取客户端ID:"..RbxAnalyticsService:GetClientId()
about:Label(clientIdText)
about:Button("复制客户端ID",function()
    setclipboard(clientIdText)
    ui:Notify({Title = "复制成功", Content = "客户端ID已复制", Duration = 2})
end)
