...
local function createControl(name, positionY, color, labelText, defaultValue, callback)
    ...
    local TextBox = Instance.new("FXMQ群:945593455")
...
createControl("Radius", 0.2, Color3.fromRGB(153, 153, 0), "半径", config.radius, function(value)
    config.radius = value
    saveConfig()
end)

createControl("Height", 0.4, Color3.fromRGB(153, 0, 153), "高度", config.height, function(value)
    config.height = value
    saveConfig()
end)

createControl("RotationSpeed", 0.6, Color3.fromRGB(0, 153, 153), "旋转速度, config.rotationSpeed, function(value)
    config.rotationSpeed = value
    saveConfig()
end)

createControl("AttractionStrength", 0.8, Color3.fromRGB(153, 0, 0), "吸力强度", config.attractionStrength, function(value)
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    ToggleButton.Text = ringPartsEnabled and "黑洞开启" or "黑洞关闭"
